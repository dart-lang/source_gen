...
void main() {
  ...
  test('Simple Generator test for library', _simpleTestForLibrary);

  test('Simple Generator test for classes and library',
      _simpleTestForClassesAndLibrary);
...
Future _simpleTestForLibrary() => _generateTest(
    const CommentGenerator(forClasses: false, forLibrary: true),
    _testGenPartContentForLibrary);

Future _simpleTestForClassesAndLibrary() => _generateTest(
...
Future _generateTest(CommentGenerator gen, String expectedContent) async {
  ...
  var relativeFilePath = p.join('lib', 'test_lib.dart');
  var output =
      await generate(projectPath, [gen], changeFilePaths: [relativeFilePath]);
  ...
  await d
    ...
    d.dir('lib', [
      d.file('test_lib.dart', _testLibContent),
      d.file('test_lib_part.dart', _testLibPartContent),
      d.matcherFile('test_lib.g.dart', contains(expectedContent))
    ])
  ])
      .validate();
...
const _testGenPartContentForLibrary = r'''part of test_lib;
...
// Code for test_lib''';
const _testGenPartContentForClassesAndLibrary = r'''part of test_lib;
